...
#define MAX_IMAGE_DATA_SIZE (4 * 2048 * 3072)
    ...
    public:
        ...
        bool reset();
        //********** 读图10个相机的原始图像 ********************************/
        void ReadImg(cv::Mat &image, uint64_t &timestamp);
    ...
    Camera::Camera(ros::NodeHandle &node)
    {
        ...
        nRet = MV_CC_SetEnumValue(handle, "PixelFormat", 0x01080009); // 目前 BayerRG8  
        ...
        if (MV_OK == nRet)
        {
            printf("set ExposureAuto OK ! value = RGB\n");
        }
        else
        {
            printf("set ExposureAuto fail! nRet [%x]\n", nRet);
        }
        MVCC_ENUMVALUE t = {0};
        //********** frame **********/
    ...
    //^ ********************************** Camera constructor************************************ //
    void Camera::ReadImg(cv::Mat &image, uint64_t &timestamp)
    {
    ...
    void *Camera::HKWorkThread(void *p_handle)
    {
        ...
        while (ros::ok())
        {
            ...
            cv::cvtColor(cv::Mat(stImageInfo.nHeight, stImageInfo.nWidth, CV_8UC1, m_pBufForDriver),
                         camera::frame,
                         cv::COLOR_BayerRG2RGB);
            
            //devTimestamp = ((uint64_t)stImageInfo.nDevTimeStampHigh << 32) | ((uint64_t)stImageInfo.nDevTimeStampLow);  // qwq
            devTimestamp = stImageInfo.nHostTimeStamp;
            frame_empty = 0;
            pthread_mutex_unlock(&mutex);
